...
local trget = table_rawget; //cache global to local
...
debugLog("Starting " + APP_CODE_FOLDER);

AT_DEV_DBG <- trget(globals, "AT_DEV_DBG", true);
//AT_DEV_DBG <- false;
...
function getUserCallbackSetup(fn){
	...
	if (trget(globals, "AT_DEV_DBG", false)){
...
/*
			...
			if(AT_DEV_DBG || !table_get(this, "handle_request", false)) {
				local plugin_fn = APP_CODE_FOLDER + "/sq-server-plugin.nut";
...
function appServerStart(port, document_root, num_threads=1){
	mg = Mongoose();
	mongoose_start_params.num_threads <- num_threads.tostring();
