class SQLiteUtils {
	...
	static function checkDbAttachments(db, the_db_file_name)
	{
		try
		{
			auto db_path = the_db_file_name.gsub("[^/]+$", "");
			if(db.exec_get_one("select exists(select 1 from sqlite_master where type='table' and name='use_attached_dbs')"))
			{
				auto stmt = db.prepare("select dbfname, alias from use_attached_dbs");
				auto stmt_attach = db.prepare("attach database ? as ?");
				while(stmt.next_row())
				{
					auto dbfname = stmt.col(0);
					if(dbfname.indexOf("/") < 0) dbfname = db_path + dbfname;
					auto alias = stmt.col(1);
					auto rc = db.exec_one_dml("attach database ? as ?", dbfname, alias);
					//print("attached_databases", rc);
				}
				db.exec_dml("pragma use_attached_dbs=ON");
			}

			if(db.exec_get_one("select exists(select 1 from sqlite_master where type='table' and name='load_temp_db')"))
			{
				auto stmt = db.prepare("select dbfname from load_temp_db");
				auto temp_dbs = [];
				while(stmt.next_row())
				{
					auto dbfname = stmt.col(0);
					if(dbfname.indexOf("/") < 0) dbfname = db_path + dbfname;
					temp_dbs.append(dbfname);
				}
				foreach(dbfname in temp_dbs)
				{
					dbfname = dbfname.replace(".sql", ".db")
					auto rc = db.restore(dbfname, "main", "temp");
					//auto sql = readfile(dbfname);
					//auto rc = db.exec_dml(sql);
					//print("load_temp_db", rc);
				}
				db.exec_dml("pragma use_attached_dbs=ON");
			}
		}
	...
	static function getDbFor(dbname, doCheckAttachments=false)
	{
		...
		if(!db)
		{
			...
			if(doCheckAttachments)	checkDbAttachments(db, dbname);
			table_rawset(__db_cache, dbname, db);
			local customExtensionDBInit = table_rawget(getroottable(), "customExtensionDBInit", false);
			if(customExtensionDBInit) customExtensionDBInit(db);			
		}
		return db;
	}
	...
	static function exec_get_row_as_table(db, sql, bind_values=null)
	{
		local result = null;
		local stmt = create_stmt_bind(db, sql, bind_values);
		if(stmt.next_row())
	...
	static function getExtraHtmlForTableName(db, the_table_name, attached_db="")
	{
		return exec_get_one(db, format("SELECT extra_html FROM %s__tables_metadata WHERE name =?", attached_db), [the_table_name]);
	}
	...
	static function createQuery(db, tbl, query_type, max_rows, attached_db=""){
		if(db){
			local sql;
			local fields, fields_csv;
			try {
				fields = getFieldsFor(db, tbl, attached_db);
			...
			if(query_type == "select")
			{
				...
				sql = format("--DROP VIEW %s\"%s_list_view\";\n--CREATE VIEW %s\"%s_list_view\" AS\nSELECT\n\t%s\nFROM %s\"%s\" AS a\nLIMIT %d", 
					attached_db, tbl, attached_db, tbl, fields_csv, attached_db, tbl, max_rows.tointeger());
				sql += myjoins;
			}
		...
		foreach(idx, tbl in all_tables)
		{
			...
			//if(!is_view)
			{
				...
				foreach(fld in all_fields)
				{
					...
					local field_type = fld.type.gsub("%s*%(.+$", "");
	...
	static function doCleanTableMetadataFieldsFor(db, all_tables, attached_db="")
	{
		...
		local updateTablesMetadata = function()
		{
			...
			local sql = format("UPDATE %s__tables_metadata as a set list_table_id=NULL WHERE NOT EXISTS(%s)", attached_db, format(sql_exists, "list_table_id"));
			db.exec_dml(sql);
			sql = format("UPDATE %s__tables_metadata as a set edit_table_id=NULL WHERE NOT EXISTS(%s)", attached_db, format(sql_exists, "edit_table_id"));
			db.exec_dml(sql);
			sql = format("UPDATE %s__app_menu as a set table_view_id=NULL WHERE NOT EXISTS(%s)", attached_db, format(sql_exists, "table_view_id"));
			db.exec_dml(sql);
		}
		...
		local sql = format("DELETE FROM %s__tables_metadata as a WHERE NOT EXISTS(SELECT 1 FROM %ssqlite_master sm WHERE sm.name=a.name)",
			attached_db, attached_db);
		db.exec_dml(sql);
	}
...
class SquiLuCode
{
	...
	function createSquiLuMainTable(db)
	{
		local sql = [==[
CREATE TABLE IF NOT EXISTS __app_squilu (
	id INTEGER PRIMARY KEY NOT NULL,
	_version_ INTEGER NOT NULL DEFAULT 0,
	_cdate_ DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
	_mdate_ DATETIME,
	name VARCHAR NOT NULL COLLATE NOCASE
	CONSTRAINT __app_squilu_name_unique UNIQUE,
	code TEXT COLLATE NOCASE NOT NULL,
	notes TEXT COLLATE NOCASE
);

CREATE  VIEW IF NOT EXISTS __app_squilu_list_view AS
SELECT
	a.id,
	a._mdate_,
	a.name
FROM __app_squilu AS a;
	...
	function doPrepare(db)
	{
		if(!stmt_get)
		{
			stmt_get = db.prepare("SELECT code FROM __app_squilu WHERE id=?");
			stmt_get_by_name = db.prepare("SELECT code FROM __app_squilu WHERE name=?");
			stmt_set = db.prepare("UPDATE __app_squilu SET code=? WHERE id=?");
		}
	}
	...
	function getSquiLuCodeName(db, sql)
	{
		createSquiLuMainTable(db);
		return sql.match("^//name=(%S+)");
	}
	
	function getSquiLuCode(db, sql)
	{
		local name = getSquiLuCodeName(db, sql);
		if(name)
		{
			return db.exec_get_one("SELECT code FROM __app_squilu WHERE name=?", name);
		}
		return name;
	}

	function insertSquiLuCode(db, code)
	{
		local name = getSquiLuCodeName(db, code);
		if(name)
		{
			db.exec_one_dml("insert into __app_squilu(name, code) values(?,?)", name, code);
			//print(db.errmsg());
		}
		return name;
	}

	function updateSquiLuCode(db, code)
	{
		local name = getSquiLuCodeName(db, code);
		if(name)
		{
			db.exec_one_dml("update __app_squilu set code = ? where name = ?", code, name);
		}
		return name;
	}
}
